...
function buildSshCommand(
  ...
  parts.push(
    `cd "${ws}" && WORKSPACE_BASE="${ws}" AGENT_BROWSER_EXECUTABLE_PATH="${browserPath}" AGENT_BROWSER_HEADED=1 BROWSER_LANG="${process.env.BROWSER_LANG || 'zh-CN'}" BROWSER_TIMEZONE="${process.env.BROWSER_TIMEZONE || 'Asia/Shanghai'}" QG_AUTH_KEY="${process.env.QG_AUTH_KEY || ''}" QG_AUTH_PWD="${process.env.QG_AUTH_PWD || ''}" QG_DEFAULT_AREA="${process.env.QG_DEFAULT_AREA || ''}" QG_DEFAULT_ISP="${process.env.QG_DEFAULT_ISP || ''}" PATH="${ws}/tools/node_modules/.bin:${ws}/tools:/Users/${LUME_VM_USER}/local/bin:/opt/homebrew/bin:$HOME/local/bin:$PATH" node "${ws}/agent-runner/dist/index.js"`,
  );
